...
app.addHook('preHandler',async(req,reply)=>{
  ...
  const token=cookieValue(req.headers.cookie,authCookie);const user=await userForToken(token);if(!user)return reply.code(401).send({error:'Authentication required'});req.authUser=user;req.authToken=token;
});
...
app.post('/api/sessions',async(req,reply)=>{ const input=sessionInput.parse(req.body); if(input.repoUrl)validateRepoUrl(input.repoUrl); const item=await db.createSession(req.authUser!.id,input); return reply.code(201).send(item); });
app.get<{Params:{id:string}}>('/api/sessions/:id',async(req,reply)=>{ const session=await db.session(req.params.id,req.authUser!.id); if(!session)return reply.code(404).send({error:'Session not found'}); return {session,runs:await db.runs(session.id)}; });
app.get('/api/connections/github',async req=>({connected:true,login:req.authUser!.login}));
app.get('/api/connections/github/repos',async(req,reply)=>{const token=await vault.getUser(req.authUser!.id,'github_token');if(!token)return reply.code(401).send({error:'GitHub authorization missing'});const response=await fetch('https://api.github.com/user/repos?per_page=100&sort=updated&affiliation=owner%2Ccollaborator%2Corganization_member',{headers:{Authorization:`Bearer ${token}`,Accept:'application/vnd.github+json','X-GitHub-Api-Version':'2022-11-28','User-Agent':'relay-cloud-agent'}});if(!response.ok)return reply.code(response.status===401?401:502).send({error:'Could not load GitHub repositories'});const repos=await response.json() as Array<{id:number;full_name:string;clone_url:string;private:boolean;default_branch:string;archived:boolean}>;return repos.filter(repo=>!repo.archived).map(repo=>({id:repo.id,name:repo.full_name,url:repo.clone_url,private:repo.private,branch:repo.default_branch}));});
...
app.get('/api/events',async(req,reply)=>{
  ...
  if(!await db.ownedRun(q.runId,req.authUser!.id))return reply.code(404).send({error:'Run not found'});
  reply.raw.writeHead(200,{'Content-Type':'text/event-stream','Cache-Control':'no-cache','Connection':'keep-alive'}); let after=q.after;
  const timer=setInterval(async()=>{ const events=await db.events(q.runId,after); for(const event of events){after=Number(event.id);reply.raw.write(`id: ${event.id}\ndata: ${JSON.stringify(event)}\n\n`);} const run=await db.run(q.runId); if(!run||['completed','failed','cancelled','interrupted'].includes(run.status)){reply.raw.write(`event: done\ndata: ${JSON.stringify(run)}\n\n`);clearInterval(timer);reply.raw.end();}},1000);
  req.raw.on('close',()=>clearInterval(timer)); return reply;
});
app.setErrorHandler((error,req,reply)=>{ const known=error as {statusCode?:number;message?:string}; const status=error instanceof z.ZodError?400:known.statusCode||500;if(status===500)req.log.error(error);reply.code(status).send({error:status===500?'Unexpected server error':known.message||'Invalid request'}); });
