...
export function runContainerName(runId:string):string{return `relay-run-${runId}`;}
...
export async function execute(run: Run, session: Session, signal: AbortSignal): Promise<string> {
  ...
  const streamDone=new Promise<void>((resolve,reject)=>{stream.on('end',resolve);stream.on('error',reject);});
  let statusCode:number|undefined;
  try{await container.start();await db.addEvent(run.id,'agent','Codex process started','Isolated run container with persistent session history');const [result]=await Promise.all([container.wait(),streamDone]);statusCode=result.StatusCode;}
  finally{signal.removeEventListener('abort',stop);try{await container.remove({force:true});}catch{}}
  if(signal.aborted)throw new Error('Run cancelled');
  if(statusCode!==0)throw new Error(output.slice(-4000)||`Agent exited ${statusCode}`);
  ...
  if(branchMarker){try{const observed=JSON.parse(branchMarker.slice('RELAY_BRANCH:'.length)) as {branch?:string};if(observed.branch&&/^[A-Za-z0-9][A-Za-z0-9._/-]{0,254}$/.test(observed.branch)&&!observed.branch.includes('..'))await db.setSessionBranch(session.id,observed.branch);}catch{}}
