...
export const db = {
    ...
    async setSessionBranch(sessionId, prBranch) { await pool.query(`UPDATE sessions SET pr_branch=$2,updated_at=now() WHERE id=$1`, [sessionId, prBranch]); },
    async replacePullRequest(sessionId, prBranch) { await pool.query(`UPDATE sessions SET pr_url=null,pr_branch=$2,updated_at=now() WHERE id=$1`, [sessionId, prBranch]); },
