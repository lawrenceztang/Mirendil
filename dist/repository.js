    ...
    if (exists) {
        if (session.prBranch) {
            ...
            try {
                ...
                await command('git', [...safe, ...auth, 'fetch', 'origin', session.prBranch], dir);
                await command('git', [...safe, 'switch', '-c', session.prBranch, '--track', `origin/${session.prBranch}`], dir);
            }
        }
        return dir;
    }
...
export async function prepareAgentOutput(runId) {
    const output = path.join(config.workspaceRoot, '.relay-outputs', runId);
    ...
    await fs.chmod(output, 0o777);
    return output;
}
export async function applyAgentOutput(workspace, output) {
    let files = 0, bytes = 0;
    const root = path.resolve(output);
    async function validate(current) { for (const entry of await fs.readdir(current, { withFileTypes: true })) {
        if (entry.name === '.git')
            throw new Error('Agent output contained forbidden .git metadata');
        const item = path.join(current, entry.name);
        const stat = await fs.lstat(item);
        if (stat.isSymbolicLink()) {
            const target = await fs.readlink(item);
            const resolved = path.resolve(path.dirname(item), target);
            if (resolved !== root && !resolved.startsWith(`${root}${path.sep}`))
                throw new Error(`Agent output contained unsafe symlink: ${path.relative(root, item)}`);
        }
            ...
            files++;
            bytes += stat.size;
            if (files > 10_000 || bytes > 200_000_000)
                throw new Error('Agent output exceeded import limits');
        }
        else
            throw new Error('Agent output contained an unsupported file type');
    } }
    await validate(root);
    ...
    for (const entry of await fs.readdir(root)) {
        await fs.cp(path.join(root, entry), path.join(workspace, entry), { recursive: true, force: true, verbatimSymlinks: true });
    }
    await fs.rm(output, { recursive: true, force: true });
}
export async function publishPullRequest(session, runId, prompt, summary, workspace, githubToken) {
    if (!githubToken || !session.repoUrl)
        return null;
    const url = validateRepoUrl(session.repoUrl);
    ...
    if (parts.length !== 2 || !parts[0] || !parts[1])
        throw new Error('GitHub repository URL must contain owner/repository');
    ...
    await git(['config', 'user.name', 'Relay Agent']);
    await git(['config', 'user.email', 'relay-agent@users.noreply.github.com']);
    if (session.prBranch)
    ...
    if (session.prUrl)
        return { url: session.prUrl, branch, created: false };
    ...
    if (!response.ok)
        throw new Error(`GitHub PR creation failed (${response.status}): ${(await response.text()).slice(0, 500)}`);
    const result = await response.json();
    if (!result.html_url)
        throw new Error('GitHub returned no pull request URL');
    return { url: result.html_url, branch, created: true };
}
