...
function databaseUrl() {
    ...
    if (process.env.DIRECT_URL) {
        ...
        if (password && /\[(?:YOUR-)?PASSWORD\]/i.test(value)) {
    ...
    if (supabaseUrl && password) {
        ...
        if (!host)
            throw new Error('SUPABASE_DB_HOST is required; copy the Session pooler host from Supabase > Connect');
        const port = process.env.SUPABASE_DB_PORT || '5432';
        return `postgresql://postgres.${project}:${encodeURIComponent(password)}@${host}:${port}/postgres`;
    }
