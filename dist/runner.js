...
async function chatContainer(session, workspace) {
    ...
    try {
        ...
        if (details.State.Running && details.Image === image.Id && hasWorkspace && hasCodexHome)
...
export async function execute(run, session, signal) {
    ...
    const prepared = await prepareAgentBranch(session, workspace, githubToken);
    const branch = prepared.branch;
    if (prepared.replacedPullRequest) {
        await db.replacePullRequest(session.id, branch);
        session.prUrl = null;
        session.prBranch = branch;
        await db.addEvent(run.id, 'setup', 'Starting a new pull request', `The previous pull request is no longer open; using ${branch}`);
    }
    else if (!session.prBranch) {
        await db.setSessionBranch(session.id, branch);
        session.prBranch = branch;
    }
    else if (session.prUrl)
        await db.setPullRequest(run.id, session.prUrl);
    const chat = await chatContainer(session, workspace);
    const container = chat.container;
    await makeAgentWritable(workspace, !chat.reused || prepared.replacedPullRequest);
    if (!chat.reused)
        await db.addEvent(run.id, 'setup', 'Starting new chat container', `${config.agentImage} · ${session.repoUrl || 'Blank workspace'}`);
    ...
    if (result.ExitCode !== 0)
        throw new Error(output.slice(-4000) || `Agent exited ${result.ExitCode}`);
